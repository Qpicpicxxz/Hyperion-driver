...
#define SRC_MAP_SIZE 1024
#define DST_MAP_SIZE 1024
...
int main(int argc, char* argv[]) {
  ...
  token.dst_offset = 0;
  token.size       = 1024;
  ioctl(fd_dma, DMA_MEM2MEM, &token);
  ...
  printf("trans 1024, cmp 1024000\n");
  int res = memcmp(dst, src, 1024);
  printf("cmp result: %d\n", res);
  printf("Trans done! 1130\n");
