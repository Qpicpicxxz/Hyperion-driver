...
#define CHAR_DEV_NAME "pl_leds_dev"
...
static ssize_t char_dev_write(struct file *file_p, const char __user *buf, size_t len, loff_t *loff_t_p) {
  ...
  if (ret != 0) {
    printk("pl_leds_dev char_dev_write copy_from_user failed\n");
    return -1;
  }
  if (len != 1) {
    printk("pl_leds_dev char_dev_write len err\n");
    return -2;
  }
  if (1 == lc_buf) {
    // printk("leds off!\n");
    ...
    printk("pl_leds_dev char_dev_write data err\n");
  }
  return 0;
}
...
static int __init char_drv_init(void) {
  ...
  leds_char.device->of_node = of_find_node_by_path("/gpio78leds");
  if (IS_ERR(leds_char.device->of_node)) {
    ret = PTR_ERR(leds_char.device->of_node);
    printk(KERN_ERR "of_find_node_by_path\n");
    return ret;  // 退出初始化过程
  }
  leds_char.gpio_d = gpiod_get_index(leds_char.device, "led78", 0, GPIOD_OUT_HIGH);
